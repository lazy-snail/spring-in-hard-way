    ...
    */
	...
	public static List<Vm> createVmList(int brokerId, List<Cloudlet> cloudlets) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			vms.add(new PowerVm(i,brokerId,VM_MIPS[vmType],VM_PES[vmType],VM_RAM[vmType],VM_BW,VM_SIZE,1,"Xen",
					...
					myPolicy.Constants.SCHEDULING_INTERVAL));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % myPolicy.Constants.HOST_TYPES;

			List<Pe> peList = new ArrayList<Pe>();
			for (int j = 0; j < myPolicy.Constants.HOST_PES[hostType]; j++) {
				peList.add(new Pe(j, new PeProvisionerSimple(myPolicy.Constants.HOST_MIPS[hostType])));
			}

			hostList.add(new PowerHostUtilizationHistory(
					i,
					new RamProvisionerSimple(myPolicy.Constants.HOST_RAM[hostType]),
					new BwProvisionerSimple(myPolicy.Constants.HOST_BW),
					myPolicy.Constants.HOST_STORAGE,
					peList,
					new VmSchedulerTimeSharedOverSubscription(peList),
					myPolicy.Constants.HOST_POWER[hostType]));
		}
		return hostList;
	}
		...
		for (int i = 0; i < files.length; i++) {
            ...
            try {
                cloudlet = new Cloudlet(
                        i,
                        myPolicy.Constants.CLOUDLET_LENGTH,
                        myPolicy.Constants.CLOUDLET_PES,
                        fileSize,
                        outputSize,
