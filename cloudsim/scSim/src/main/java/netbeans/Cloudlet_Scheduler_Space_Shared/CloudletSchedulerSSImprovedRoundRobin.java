...
public class CloudletSchedulerSSImprovedRoundRobin extends CloudletScheduler {
    ...
    public CloudletSchedulerSSImprovedRoundRobin() {
        super();
    ...
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
       ...
       // timeSpan = TIME_QUANTA_SIZE * no_of_quantas;
        ...
        long finished = 0, instructions_in_one_quanta = (long) (capacity * TIME_QUANTA_SIZE * Consts.MILLION);
        List<ResCloudlet> toRemove = new ArrayList<>();
        for (ResCloudlet rcl : getCloudletExecList()) {
            // finished anyway, rounding issue...
            //this part has been implemented to make the RR improved
            if (rcl.getRemainingCloudletLength() == 0
                    || rcl.getRemainingCloudletLength() <= instructions_in_one_quanta * rcl.getNumberOfPes()) {
                if (rcl.getRemainingCloudletLength() > 0) {
