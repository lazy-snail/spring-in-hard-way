 ...
 * and open the template in the editor.
 */
package netbeans.Cloudlet_Scheduler_Time_Shared;
 ...
 * @author Tamojit9
 */
class ResCloudletComparator1 implements Comparator<ResCloudlet>{

    @Override
    public int compare(ResCloudlet t, ResCloudlet t1) {
        return t.getRemainingCloudletLength() >= t1.getRemainingCloudletLength() ? -1 : 1;
    }
    
}

public class CloudletSchedulerTSSRTFImprovedWithIRR extends CloudletScheduler {
    ...
    private double TIME_QUANTA_SIZE;
     ...
     * @post $none
     */
    public CloudletSchedulerTSSRTFImprovedWithIRR() {
        super();
    ...
    public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
        ...
        if(!sortedExec) {
                    Collections.sort(getCloudletExecList(), new ResCloudletComparator1());
                    sortedExec = true;
        }
        for (ResCloudlet rcl : getCloudletExecList()) {
