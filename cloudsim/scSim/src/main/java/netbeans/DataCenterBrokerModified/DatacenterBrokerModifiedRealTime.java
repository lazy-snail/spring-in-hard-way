...
public final class DatacenterBrokerModifiedRealTime extends SimEntity {
    ...
    //==============================================================REAL TIME========================================
    List<Cloudlet> queueBuffer = new LinkedList<>();
    PriorityQueue<VmComparator> checkList = new PriorityQueue<>();
    int vmInd = 0, startTime = 0;
    volatile int presentTime = 0;
    boolean sort = false;
    final int TimeUnit = 10; // means 100 milliSecs = 1 time unit
    Timer updatePresentTime;
    
    Cloudlet pop() {
        //pop from the FIFO queueBuffer
        return queueBuffer.remove(0);
    }
    
    public void submitCloudlet(Cloudlet c) {
        int finishTime;
        if(!sort)  {
            sortVmList();
        }
        if(checkList.isEmpty()) {
            VmComparator vm = (VmComparator) vmList.get(vmInd++);
            finishTime = presentTime + (int) (c.getCloudletLength()/vm.getMips());
            bindCloudletToVm(c.getCloudletId(), vm.getId());
            vm.estimatedTime = finishTime;
            checkList.add(vm);
        } else {
    ...
    public void startTimer() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while(count-- > 0) {
                    try {
                        Thread.sleep(TimeUnit*(int)(Math.random()*10));
                        submitCloudlet(cloudletList.get(--count));
                    } catch (InterruptedException ex) {
                        Logger.getLogger(DatacenterBrokerModifiedRealTime.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        }).start();
        updatePresentTime = new Timer(TimeUnit, new ActionListener() {
        ...
        updatePresentTime.start();
    }
    
    public void stopTimer() {
        updatePresentTime.stop();
    }
     ...
     * @post $none
     */
    public DatacenterBrokerModifiedRealTime(String name) throws Exception {
        super(name);
    ...
    int roundOff(double stripLength) {
        double decPart = stripLength - (int)stripLength;
        return decPart > 0.5 ? (int)stripLength+1 : (int)stripLength;
    }
    ...
    protected final <T extends Cloudlet> void setCloudletList(List<T> cloudletList) {
        this.cloudletList = (List<Cloudlet>) cloudletList;
    }
