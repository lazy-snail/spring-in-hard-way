...
public final class DatacenterBrokerModifiedConductance extends SimEntity {
     ...
     * @post $none
     */
    public DatacenterBrokerModifiedConductance(String name) throws Exception {
        super(name);
    ...
    public void bindByLoadBalancer() throws Exception {
        ...
        double conductance, length = cloudletList.size();
        int from = cloudletList.size() - 1;
        for(Vm vm : vmList) {
            conductance = vm.getMips()*1.0/totalMips;
            int range = roundOff(conductance*length);
            System.err.println(range);
            while(range-- > 0 && from > 0) bindCloudletToVm(cloudletList.get(from--).getCloudletId(), vm.getId());
        }
    }
